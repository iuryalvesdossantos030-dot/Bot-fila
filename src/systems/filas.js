...
async function entrar1v1(tipo, interaction) {
  ...
  // ✅ Responde imediatamente para evitar "This interaction failed"
  ...
  if (!data.filas["1v1"][tipo].includes(userId)) {
    ...
    await interaction.editReply("✅ Você entrou na fila 1v1!");
  } else {
    await interaction.editReply("⚠️ Você já está nessa fila.");
  }

  await verificarMatch1v1(interaction.guild, tipo);
}
